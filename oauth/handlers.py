...
class AuthorizationHandler(webapp.RequestHandler):
    ...
    def authorize(self, client_id):
        """
        Authorizes a client for access to the resource.  Returns None if authorization failed, or else a redirect_uri
        for the relevant RP.
        :param client_id:
        :return:
        """
    ...
    def validate_params(self):
        ...
        redirect_uri = self.authorize(self.request.get('client_id'))
        if redirect_uri is None:
            self.authz_error('access_denied', "The client does not have access to the requested resource")
...
class AccessTokenHandler(webapp.RequestHandler):
    ...
    def authenticate_client(self, client_id, client_secret):
        """Authenticate a user.  Override this."""
    ...
    def handle(self):
        ...
        getattr(self, 'handle_%s' % grant_type)(client_id, scope)
    
    def handle_password(self, client_id, scope=None):
        ...
        if not username or not password:
            ...
            return
        success, extra = self.authenticate_user(username, password)
        if not success:
            self.render_error('invalid_grant', "Invalid end-user credentials.")
            return
    ...
    def handle_client_credentials(self, client_id, scope=None):
        token = OAuth_Token(
            client_id   = client_id,
            scope       = scope, )
        token.put(can_refresh=False)
